...
Public Class MenuStreamingService
    ...
    Public Sub Dummy(poPar As List(Of TelerikMenuBackCls.MenuDTOnon), ByVal poPar1 As List(Of MenuDashboardDTOnon)) Implements IMenuStreamingService.Dummy
    ...
    Public Function GetDashBoardMenu() As Message Implements IMenuStreamingService.GetDashBoardMenu
        Dim loException As New R_Exception
        Dim loRtnTemp As List(Of MenuDashboardDTOnon) = Nothing
        Dim loRtn As Message = Nothing

        Try
            Dim loCls As New MenuCls
            Dim lcCompId As String = R_Utility.R_GetStreamingContext("cCompId")
            Dim lcUserId As String = R_Utility.R_GetStreamingContext("cUserId")

            loRtnTemp = loCls.GetDashBoardMenu(lcCompId, lcUserId)

            loRtn = R_StreamUtility(Of MenuDashboardDTOnon).WriteToMessage(loRtnTemp.AsEnumerable, "GetDashBoardMenu")
        Catch ex As Exception
            loException.Add(ex)
        End Try
