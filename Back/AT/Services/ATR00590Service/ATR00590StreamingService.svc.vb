Imports R_Common
Imports System.ServiceModel.Channels
Imports ATR00590Back
Imports ATR00590Common
Imports R_BackEnd
' NOTE: You can use the "Rename" command on the context menu to change the class name "ATR00590StreamingService" in code, svc and config file together.
Public Class ATR00590StreamingService
    Implements IATR00590StreamingService

    Public Function getReport() As System.ServiceModel.Channels.Message Implements IATR00590StreamingService.getReport
        Dim loException As New R_Exception
        Dim loCls As New ATR00590CLS
        Dim poParam As New ATR00590DTO
        Dim loRtnList As List(Of ATR00590ReportDTO)
        Dim loRtn As Message
        Dim loList As List(Of Byte())

        Try
            With poParam
                .cCompanyId = R_Utility.R_GetStreamingContext("cCompanyId")
                .iAllOvertime = R_Utility.R_GetStreamingContext("iAllOvertime")
                .cOvertimeType = R_Utility.R_GetStreamingContext("cOvertimeType")
                .cFromDate = R_Utility.R_GetStreamingContext("cFromDate")
                .cToDate = R_Utility.R_GetStreamingContext("cToDate")
                .iAllStatus = R_Utility.R_GetStreamingContext("iAllStatus")
                .cTransStatus = R_Utility.R_GetStreamingContext("cTransStatus")
                .iAllEmployee = R_Utility.R_GetStreamingContext("iAllEmployee")
                .cEmployeeList = R_Utility.R_GetStreamingContext("cEmployeeList")
                .cUserId = R_Utility.R_GetStreamingContext("cUserId")
                .cLanguageId = R_Utility.R_GetStreamingContext("cLanguageId")
                .cReportType = R_Utility.R_GetStreamingContext("cReportType")
            End With

            loRtnList = loCls.getReport(poParam)
            loList = R_Utility.R_GetChunkData(Of ATR00590ReportDTO)(loRtnList, R_BackGlobalVar.CHUNK_SIZE)
            loRtn = R_StreamUtility(Of Byte()).WriteToMessage(loList.AsEnumerable, "getReport")
        Catch ex As Exception
            loException.Add(ex)
        End Try

        loException.ConvertAndThrowToServiceExceptionIfErrors()

        Return loRtn
    End Function
End Class
